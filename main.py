...
# КРЧ вот описание проекта:
# ТГ БОТ в который вводятся два значения (1 резистор и 2 резистор) исходя из этих значений вычисляется общее сопротивление в последовательном и паррарельном подключении
# Это все голишь болванка, она работает. Но я не называю это ботом, это отброс которому нужна доработка, не надейтесь доработать на 100%, старайтесь добавлять новые функции аккуратно и делайте это в асбтракции.
# Крч распределяйте все функции и классы раздельно, так будет лучше и даваайте договоримся, не засоряйте main)
